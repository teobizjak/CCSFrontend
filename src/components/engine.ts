...
export default class Engine {
  ...
  private transformSFMessageData(e) {
    ...
    if (!uciMessage.includes(" multipv 3 ") && !uciMessage.includes(" multipv 2 ")) {
        var positionEvaluation = uciMessage.match(/cp\s+(\S+)/)?.[1];
        var possibleMate = uciMessage.match(/mate\s+(\S+)/)?.[1];
    }
    return {
      uciMessage,
